...
test('Нагрузочный тест', async ({ browser }) => {
  test.setTimeout(300000);

  const usersCount = 10;
  // Допустимый процент ошибок (например, 10%)
  ...
  const userTasks = Array.from({ length: usersCount }, async (_, index) => {
    ...
    try {
      ...
      await page.waitForTimeout(1000); // небольшая пауза для завершения поиска
      searchTime = Date.now() - searchStart;

      const totalUserTime = Date.now() - userStartTime;

      // ------------------------------------------------------------
      // 🔥 ДОБАВЛЯЕМ ЗАДЕРЖКУ ПЕРЕД ЗАКРЫТИЕМ КОНТЕКСТА
      // Это помогает избежать ошибки apiRequestContext._wrapApiCall
      // ------------------------------------------------------------
      ...
      // И в блоке catch тоже добавляем задержку перед закрытием
      await page.waitForTimeout(100);
      await context.close();
      return {
        userNumber,
  ...
  // ============================================
  // ПРОВЕРКИ (с допустимым процентом ошибок)
  // ============================================
